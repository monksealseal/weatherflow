...
import { AmbientLight, AxesHelper, CanvasTexture, Color, DirectionalLight, Group, Line, LineBasicMaterial, Mesh, MeshPhongMaterial, MeshStandardMaterial, PerspectiveCamera, PlaneGeometry, Points, PointsMaterial, Scene, SphereGeometry, Vector3, WebGLRenderer } from '../vendor/three-lite';
const ORBITAL_DISTANCE = 28;
const PILOT_DISTANCE = 12;
const createIntersectionLine = (height) => {
...
const createSlicePlane = (width, height, color, position, rotation, overlayTexture) => {
...
const createVolumetricGroup = () => {
    ...
    const particles = new Points(new PlaneGeometry(24, 24, 64, 64), new PointsMaterial({
        color: '#e0f2fe',
        size: 0.06,
        transparent: true,
        opacity: 0.4
...
const AtmosphereViewer = () => {
    const containerRef = useRef(null);
    const rendererRef = useRef(null);
    const sceneRef = useRef(null);
    const pilotCameraRef = useRef(null);
    const orbitalCameraRef = useRef(null);
    const [mode, setMode] = useState('orbital');
    const [overlay, setOverlay] = useState('temperature');
    ...
    useEffect(() => {
        ...
        // Earth scaffold
        const globe = new Mesh(new SphereGeometry(5, 48, 32), new MeshPhongMaterial({
        ...
        const lonHeightPlane = createSlicePlane(sliceWidth, sliceHeight, '#be123c', new Vector3(0, 0, 0), new Vector3(0, Math.PI / 2, 0), overlayTexture ?? undefined);
        // Intersection tracer
    ...
    return (_jsxs("section", { className: "section-card game-viewer", children: [_jsxs("div", { className: "game-viewer__header", children: [_jsxs("div", { children: [_jsx("h2", { children: "Atmospheric game scaffold (Phase 1)" }), _jsx("p", { children: "Three.js scene with slice planes, intersection tracer, and dual camera modes." })] }), _jsxs("div", { className: "game-viewer__actions", children: [_jsx("button", { type: "button", className: mode === 'orbital' ? 'primary-button' : 'ghost-button', onClick: () => setMode('orbital'), children: "Satellite view" }), _jsx("button", { type: "button", className: mode === 'pilot' ? 'primary-button' : 'ghost-button', onClick: () => setMode('pilot'), children: "Pilot view" })] })] }), _jsxs("div", { className: "game-viewer__controls", children: [_jsxs("label", { className: "control-field", children: ["Overlay", _jsxs("select", { value: overlay, onChange: (event) => setOverlay(event.target.value), children: [_jsx("option", { value: "temperature", children: "Temperature shading" }), _jsx("option", { value: "moisture", children: "Moisture shading" }), _jsx("option", { value: "none", children: "No overlay" })] })] }), _jsxs("label", { className: "control-field slider", children: ["Slice tilt", _jsx("input", { type: "range", min: -0.6, max: 0.6, step: 0.02, value: sliceTilt, onChange: (event) => setSliceTilt(Number(event.target.value)) })] })] }), _jsx("div", { className: "game-viewer__canvas", ref: containerRef, children: fallbackMessage && _jsx("div", { className: "game-viewer__fallback", children: fallbackMessage }) })] }));
};
