...
class PhysicsLossCalculator(nn.Module):
    ...
    def compute_pv_conservation_loss(
        ...
        # Spatial spacing
        # Validate grid dimensions to prevent division by zero and degenerate derivatives
        if lat_dim < 2:
            raise ValueError(
                f"lat_dim must be >= 2 for spatial derivatives, got {lat_dim}"
            )
        if lon_dim < 2:
            raise ValueError(
                f"lon_dim must be >= 2 for spatial derivatives, got {lon_dim}"
            )
